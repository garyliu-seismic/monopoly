...
- **银行面板**：右侧显示所有玩家存贷，可存款（每圈 5% 利息）、取款、贷款（每圈 10% 利息，逾期 20%，上限 ¥10000）、还款。
- **存档 / 读档**：菜单“游戏 → 存档 / 读档”，把整局（玩家、棋盘、股票、银行、随机数状态）保存为 JSON 并随时恢复。
...
- **监狱 / 进房**：停靠在"进房"（GO_JAIL）触放入狱；入狱后掷骰得到对子立刻出狱，否则按天累计，满 `JAIL_MAX_TURNS`（3 天）后可支付 `BAIL_COST`（1000 元）赎金出狱，无力支付则再停留一天。
- **税收**：经过"税收"格按固定金额缴税。
- **股票交易**：四支股票（科技 / 银行 / 能源 / 地产）每回合价格随机波动（±50），玩家可用现金买卖，赚取差价。
- **银行**：存款每圈（经过起点）+5% 利息，贷款每圈 +10% 利息，连续 3 圈未还则逾期利率翻倍至 20%（贷款上限 ¥10000）；现金与存款均为 0 才判定破产。
...
- `test_bank_deposit_and_withdraw` / `test_bank_borrow_and_repay` — 银行存款/贷款
- `test_bank_interest_accrues_each_turn` / `test_bank_loan_limit` — 银行利息与上限
